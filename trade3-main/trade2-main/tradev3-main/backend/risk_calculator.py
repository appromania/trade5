...
class RiskCalculator:
    ...
    def calculate_risk_reward(self, lookback_days: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            # Critical fix: SL must never be above entry or equal to current price
            sl_atr_based = current_price - (atr * 1.5)
            sl_support_based = support - (atr * 0.5)
            
            # Choose the higher of the two, but ensure it's below current price
            stop_loss = max(sl_atr_based, sl_support_based)
            
            # Safety check: if SL is still too close or above price, recalculate
            if stop_loss >= current_price * 0.95:
            ...
            # If price is already above resistance, find next level
            if current_price >= resistance * 0.98:
                # Price near or above resistance - recalculate TP
                take_profit = current_price + (atr * 3)
            else:
                # Normal case: TP slightly below resistance
