...
export default function CandlestickChart({ 
  ...
  const processedData = useMemo(() => {
    ...
    const candles = dataToUse.map(item => ({
      time: item.time, // Keep as YYYY-MM-DD string
  ...
  const donchianData = useMemo(() => {
    ...
    const sortedData = [...donchianChannel.channel_data].sort((a, b) => a.time.localeCompare(b.time));
  ...
  useEffect(() => {
    ...
    if (donchianData.upper.length > 0) {
      ...
      upperLine.setData(donchianData.upper);
      ...
      lowerLine.setData(donchianData.lower);
      ...
      middleLine.setData(donchianData.middle);
    }
