...
class AIAnalyzer:
    ...
    async def analyze(
        ...
        if not self.client:
            ...
            return self._generate_fallback_analysis(symbol, indicators, signal, risk_data, fundamentals, price_change_percent)
        
        try:
            analysis_context = self._build_context(symbol, indicators, risk_data, signal, context, alerts, fundamentals, price_change_percent)
...
Evită generalitățile - oferă un plan de acțiune clar pe care traderul îl poate executa imediat."""
    ...
    def _build_context(
        ...
        lines.append(f"📉 SCHIMBARE ZILNICĂ: {price_change_percent:+.2f}%")
        
        # CRITICAL ALERT for massive drops
        if price_change_percent < -8.0:
            lines.append(f"\n⚠️⚠️⚠️ ALERTĂ CRITICĂ: PRĂBUȘIRE ZILNICĂ DE {price_change_percent:.1f}%! ⚠️⚠️⚠️")
            lines.append("Acesta este un semnal DE PERICOL major. Analiza trebuie să reflecte gravitatea situației.\n")
    ...
    def _generate_fallback_analysis(
        ...
        # CRITICAL: Check for massive drop FIRST
        if price_change_percent < -8.0:
            analysis_parts.append(f"• 🚨 **ALERTĂ CRITICĂ**: PRĂBUȘIRE ZILNICĂ de {price_change_percent:.1f}%!")
            analysis_parts.append(f"• ⚠️ **RISC MAJOR**: Momentum negativ extrem. Probabilitate mare de continuare a scăderii.")
            analysis_parts.append(f"• 🛑 **Recomandare**: EVITAȚI intrări noi până la stabilizare (min 2-3 zile de consolidare).")
        
        # Technical Assessment
        if trend == 'BULLISH' and rsi < 70 and price_change_percent >= -5:
        ...
        elif trend == 'BEARISH' or price_change_percent < -8:
            analysis_parts.append("• 📊 Tehnic: Presiune de vânzare intensă. Evitați intrări lungi până la revenire clară.")
        
        # Fundamentals - UPDATE with Debt/Equity check
        if fundamentals:
            fcf = fundamentals.get('free_cash_flow', 0)
            dte_raw = fundamentals.get('debt_to_equity', 0)
            
            # Normalize D/E to ratio format
            if dte_raw > 100:
                dte = dte_raw / 100  # Convert percentage to ratio
            else:
                dte = dte_raw
            
            fundamental_issues = []
            if fcf < 0:
                fundamental_issues.append("FCF negativ (compania arde bani)")
            if dte > 10.0:
                fundamental_issues.append(f"Debt/Equity CRITIC: {dte:.1f}x (extrem de supraleviată)")
            elif dte > 3.0:
                fundamental_issues.append(f"Debt/Equity ridicat: {dte:.1f}x")
            
            if fundamental_issues:
                analysis_parts.append(f"• 💰 Fundamentale: ⚠️ {', '.join(fundamental_issues)}. Risc financiar ridicat.")
            elif fcf > 0 and dte < 1.0:
                analysis_parts.append("• 💰 Fundamentale: Sănătate financiară solidă (FCF pozitiv, datorii moderate).")
            else:
                analysis_parts.append("• 💰 Fundamentale: Situație financiară medie.")
        else:
            analysis_parts.append("• 💰 Fundamentale: Date fundamentale indisponibile.")
        
        # Risks - UPDATE to include massive drops and debt
        risks = []
        if price_change_percent < -8.0:
            risks.append(f"Prăbușire zilnică ({price_change_percent:.1f}%)")
        if stoch_rsi > 85:
        ...
        if fundamentals:
            dte_raw = fundamentals.get('debt_to_equity', 0)
            dte_normalized = dte_raw / 100 if dte_raw > 100 else dte_raw
            if dte_normalized > 10.0:
                risks.append(f"Debt/Equity > 10x ({dte_normalized:.1f}x)")
        if fundamentals and fundamentals.get('free_cash_flow', 0) < 0:
            risks.append("Free Cash Flow negativ")
        
        if risks:
            analysis_parts.append(f"• ⚠️ Riscuri: {', '.join(risks)}. {'Evitați intrarea - prea multe semnale negative.' if len(risks) >= 3 else 'Prudență recomandată.'}")
        else:
            analysis_parts.append("• ⚠️ Riscuri: Riscuri minime detectate în prezent.")
